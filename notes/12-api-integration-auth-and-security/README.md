# Chapter 12: API (Application Programming Interface) Integration, Authentication & Security

**Status:** Not Started
**Folder:** `notes/12-api-integration-auth-and-security/`

## Why this chapter matters for a 5-10 YOE interview
"Where would you store a JWT (JSON Web Token)?" and "how do you handle a 401 mid-session" are classic senior questions that most React-only prep skips entirely.

## Topics to cover

- REST (Representational State Transfer) vs GraphQL — conceptual overview and trade-offs for a frontend consumer
...
- Token storage trade-offs: memory vs localStorage vs HttpOnly cookies, and why storage choice matters for XSS (Cross-Site Scripting) exposure
- Cookie flags: HttpOnly, Secure, SameSite
- Authorization: RBAC (Role-Based Access Control), permissions, feature flags, route protection, UI (User Interface)-level vs backend-level authorization
